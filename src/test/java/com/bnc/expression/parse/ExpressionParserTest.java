...
public class ExpressionParserTest {
    ...
    public void parse() {
        ...
        //ExpressionParser.parse("(用户=1000) and ((IP='192.168.1.x' or 地址='深圳') AND 张三='哈哈哈')");

        //(用户=1000) and ((IP='192.168.1.x' or 地址='深圳') AND (张三='哈哈哈') AND (王五='bababa') )

        // first step parse to expression array
        //String origin = "(用户=1000) and ((IP='192.168.1.x' or 地址='深圳') AND (张三='哈哈哈' OR 百花=xxx) AND (王五='bababa'))";
        String origin = "用户=1000 and IP='192.168.1.x' or 地址='深圳' AND (张三='哈哈哈' OR 百花=xxx) AND (王五='bababa')";

        // 1.解析
        Expression[] expressions = parseOrigin(origin);
        //System.out.println(JSONUtil.toJsonStr(expressions));
    ...
    private void replace(Expression[] expressions) {
     ...
     */
    private void parseExpression(Stack<Expression> stack) {
        ...
        if (!subStack.isEmpty()) {
            ...
            for (int i = 0; i < subExpressions.size(); i++) {
                Expression sub = subExpressions.get(i);
    ...
    private Stack<Expression> subStack(Stack<Expression> stack) {
    ...
    private ExpressionNode getLeft(Stack<ExpressionNode> nodeStack) {
        return nodeStack.peek() == null ? null : nodeStack.pop();
    }

    private Expression[] parseOrigin(String origin) {
       ...
       // System.out.println(JSONUtil.toJsonStr(expressionStack));
